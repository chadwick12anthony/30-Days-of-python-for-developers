...
# Change to Safari since you don't have Chrome
browser = webdriver.Safari()  # Changed from Chrome to Safari

url = "https://www.instagram.com"
browser.get(url)

time.sleep(2)
username_el = browser.find_element(By.NAME, "username")  # Updated
username_el.send_keys(INSTA_USERNAME)

password_el = browser.find_element(By.NAME, "password")  # Updated
password_el.send_keys(INSTA_PASSWORD)

time.sleep(1.5)
submit_btn_el = browser.find_element(By.CSS_SELECTOR, "button[type='submit']")  # Updated
submit_btn_el.click()

body_el = browser.find_element(By.CSS_SELECTOR, "body")  # Updated
...
# browser.find_elements(By.CSS_SELECTOR, "button")  # Updated

# xpath
# my_button_xpath = "//button"
# browser.find_elements(By.XPATH, my_button_xpath)  # Updated
...
def click_to_follow(browser):
    ...
    follow_btn_elements = browser.find_elements(By.XPATH, my_follow_btn_xpath)  # Updated
...
post_links = browser.find_elements(By.XPATH, post_xpath_str)  # Updated
post_link_el = None
...
video_els = browser.find_elements(By.XPATH, "//video")  # Updated
images_els = browser.find_elements(By.XPATH, "//img")  # Updated
...
def automate_comment(browser, content="That is cool!"):
    ...
    comment_el = browser.find_element(By.XPATH, comment_xpath_str)  # Updated
    comment_el.send_keys(content)
    submit_btns_xpath = "button[type='submit']"
    submit_btns_els = browser.find_elements(By.CSS_SELECTOR, submit_btns_xpath)  # Updated
    time.sleep(2)
...
def automate_likes(browser):
    ...
    all_like_hearts_elements = browser.find_elements(By.XPATH, like_heart_svg_xpath)  # Updated
    max_heart_h = -1
    ...
    for heart_el in all_like_hearts_elements:
        ...
        if h == max_heart_h or h == f"{max_heart_h}":
            parent_button = heart_el.find_element(By.XPATH, '..')  # Updated
            time.sleep(2)
            try:
                parent_button.click()
            except:
